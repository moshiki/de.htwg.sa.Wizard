...
case class RoundManager(numberOfPlayers: Int = 0,
  ...
  override def cardDistribution: RoundManager = {
    ...
    val playersWithCards = players map(player => {
      ...
      val cardsForPlayer = {
        val response = Http().singleRequest(Post("http://cardmodule:1234/cardStack/cardsForPlayer",
      ...
      val assignedCards = {
        val response = Http().singleRequest(Post("http://cardmodule:1234/cardStack/assignCardsToPlayer",
    ...
    Http().singleRequest(Post("http://cardmodule:1234/cardStack/splitCardStack",
  ...
  override def playerStateStrings: String = {
    ...
    if (predictionPerRound.size < numberOfPlayers) {
      val trumpColorFuture = Http().singleRequest(HttpRequest(uri = "http://cardmodule:1234/cardStack/trumpColor"))
  ...
  override def nextRound: RoundManager = {
    if (currentPlayerNumber == 0 && currentRound != numberOfRounds && players.last.playerCards.isEmpty) {
      Http().singleRequest(HttpRequest(uri = "http://cardmodule:1234/cardStack/shuffleCardStack"))
      copy(
  ...
  def trickInThisCycle: RoundManager = {
      ...
      val response = Http().singleRequest(Post("http://cardmodule:1234/cardStack/playerOfHighestCard", playedCards.reverse))
  ...
  override def topOfStackCardString: String = {
    val response = Http().singleRequest(Get("http://cardmodule:1234/cardStack/topOfCardStackString"))
